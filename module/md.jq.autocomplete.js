module.exports = {
    init: function($){
		...
		$.Autocompleter = function(input, options) {
			...
			$input.bind( "keydown.autocomplete", function(event) {
				...
				switch(event.keyCode) {
					case KEY.LEFT: // left
						if (event.shiftKey) select.firstPage();
						else            select.prevPage();
						break;
					case KEY.RIGHT: // right
						if (event.shiftKey) select.lastPage();
						else            select.nextPage();
						break;
					case KEY.UP:
                ...
                // var _obj = {'result':'aa'}
                // var _select = select.selected() || _obj;
                // var _val = _select.result.toLowerCase() || '';
			...
			function onChange(crap, skipPrevCheck) {
				//2010-01-27 by warner，添加onBegin函数，以便在启动onChange的时候，可以重新设置options
		        if (options.onBegin) {
		...
		$.Autocompleter.defaults = {
		    ...
		    scrollHeight: 300,
		    navi_class: 'ac_navi',
		    navi_num: 5,                        //页码显示页面导航数
		    navi_simple: false,                    //是否显示首页和尾页的链接
		    per_page: 10,                       //每页显示的数量
		    page:true,//是否带翻页
		    onBegin: null  //自己增加-用于动态改变参数
		};
		...
		$.Autocompleter.Select = function (options, input, select, config) {
			...
			function init() {
				...
				$navi = $('<div class="ac-more-box"><span class="ac_lefts" data-toggle="modal" data-target="#myModal"></span><span class="page-data"></span></div>')
					.addClass(options.navi_class);//在底部加上翻页
			...
			function fillList() {
				...
				var  startNum = (pageNo - 1)*options.per_page;//处理分页
				...
				if ( $.fn.bgiframe )
					list.bgiframe();
				if ( options.page )//如果需要翻页才翻页
				setNavi(data.length, listItems.length, pageNo);
			}
			//cnt:数量 cnt_page：当前页的数量 page_num：pageNo页码
			function setNavi(cnt, cnt_page, page_num) {
				...
				var cnt_result = msg['page_info']
				 ...
				 // $navi.find('.page-data').text(cnt_result);//分页重复添加
				 $navi.text(cnt_result);

				var navi_p = $('<p></p>'); //分页对象的一部分
				lastPageNo    = Math.ceil(cnt / pageSize); //总页数
			...
			return {
				...
				show: function() {
					...
					var moveLeft = typeof options.moveLeft == "string" || options.moveLeft > 0 ? options.moveLeft : 0;
